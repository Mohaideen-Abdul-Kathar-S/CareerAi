...
export interface Candidate {
  ...
  suggestions: string[];
  hidden?: boolean; // If true, candidate is rejected
}
...
export function ResultsPage({ candidates, onViewAnalysis }: ResultsPageProps) {
  const topCandidates = candidates.filter(c => c.status === 'shortlisted' && c.hidden===false);
  const rejectedCandidates = candidates.filter(c => c.status === 'rejected' || c.hidden===true);
